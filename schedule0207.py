        ...
        sys.exit()
            ...
            for i in range(len(file_to_output)):
                ...
                for j in range(size):#根據工單號
                    ...
                    #_tmp_cist_no=file_to_output[i].queue.deq()
                    ...
                    if _tmp_cist_no!=None:
                        ...
                        if temp_tuple!=None:
                            _tmp_mach=str(temp_tuple[7])#機台號
                            #%%%0122:這邊可能要換成上面的D+缸號
...
def readfile_to_tree(_rec_showlist, tree):
    ...
    global db_co_object
    db_co_object=None
    db_co_object=config()#建立DB物件，取得目前DB_config.ini目前檔案值
    ...
    _to_jobtask_list=[]
    clear_tree_content(tree)#清除目前相關tree內容，避免重復顯示
    file_to_tree=[]
    file_to_tree=_rec_showlist
    #file_to_tree=DBList#測試看看

    size=len(file_to_tree)#-----------計算碼長 並填入相關list中
    global jobtask_list#--目前會傳送的list
    jobtask_list=[]
    ...
    for j in range(size):
        ...
        if temp_emb_qty==0.0 or temp_pi_qty==0.0:#如果無投胚量或是碼長，則是都設定濃度為0.0(因為無法算出相關濃度值)
            temp_con=0.0#濃度設定為0.0
        else:
            temp_con=temp_pi_qty/(temp_emb_qty*1000)#工單各自濃度
       ...
       if _dispatched_cylinder==_temp_cy_no:
            ...
            if not queue_flag: #0115:如果queue不滿(小於8張工單)
                ...
                if queue_size>=8:#如果大於8張工單，則式進行過載平衡
                    ...
                    print("工單號 %s 已排入第%s分類 第 %s 缸中" %(_test_no,_result_class, processing_cylinder_list[i].get_cylinder_no()))
                    break
...
def load_balance(_rec_test_con, _rec_target_cylinder,_rec_class, _rec_no, _rec_cy_list, _rec_group_list, _rec_ptr_list):
    ...
    if up_level_class_ptr<=len(_rec_group_list):
        ...
        if not check_queue_is_fulled_or_not(_rec_cy_list,_target_cylinder_no):
    ...
    for index in range(_cy_group_index_size):
        ...
        for j in range(len(_pro_joblist)):
            if _temp_list_no==_pro_joblist[j].get_cylinder_no():#找到相關缸號
               _temp_list=[]
               _temp_list=_pro_joblist[j].get_cylinder_con_list()#取得已經排入的list_job工單           
               ...
               if _cy_group_index_size<3:#小於淺中深色三種，則是把淺跟中排一起，深色獨立一缸
                  if _cy_group_index_size==2:#剛好兩缸，則是分為淺中唯一缸，深色一缸
                    ...
                    if index==0:#淺與中色為一缸
                        ...
                        #0207:發現手排沒有辦法顯示正常缸list清單
                        #_pro_joblist[j].clear_cylinder_con_list()
                        #=======================================
...
def cy_selection_changed(_rec_select,_rec_tmp_list,_show_listbox, _pop_rec_info_listbox):
    ...
    for i in range(size):
        ...
        if _rec_number==temp_index:
            #_tmp_queue=copy.copy(_rec_tmp_list[i].queue)
            _tmp_list=_rec_tmp_list[i].get_cylinder_con_list()#取得list資料
            
            break
    #print_each_cylinder_queue(_tmp_queue,_show_listbox)#顯示相關點選的queue內容
...
def search_cylinder_name_from_jobno(_target_jobno):
    ...
    elif _target_jobno=="100":#設定暫時缸
        cy_name=listbox_temp
    else:
        cy_name=None
    return cy_name
#=====================從右邊已排缸號內容，把工單號碼搬移到左邊: 輸入(選擇到的工單項目，新缸號顯示區域位置，pop視窗左手邊顯示listbox，pop視窗右手邊顯示listbox，工單號，總缸list)，輸出(移動到新缸後的結果)
def move_to_left(_rec_cy_select,_cy_schedule_listbox, _left_listbox, _right_listbox,_rec_cyno,_rec_list):
    ...
    _get_right_select=_rec_cy_select.get()
    
    if _get_right_select==str(_rec_cyno):
        ...
        try:
            if  _right_listbox.curselection():
                ...
                value=_right_listbox.get(index)
        ...
        #帶入編號，找出對應index number:提供後續queue
        #------刪除後跑至顯示於左邊_left_listbox視窗中------
        #------新增點選工單至到後來的queue: ex: 9缸-----------
        ...
        print_scheduled_jobno_for_each_cylinder(_rec_list[_added_cy_index].get_cylinder_con_list(),_left_listbox)
        print_scheduled_jobno_for_each_cylinder(_rec_list[_added_cy_index].get_cylinder_con_list(),_cy_schedule_listbox)#新工單顯示listbox
 
        #-------------從原本的缸號中移除點選的工單號與濃度資料-------------------------
        _remove_cy_index=0
        ...
        #0111:此段去濾掉有工單的tuple  留下沒有工單值的tuple
...
def scheduler_to_dyeing():##: 輸入()，輸出(啟動主程式，目前包成副函式，以後可在其他物件中，呼叫此副函式，減少物件的浪費)
    ...
    btn4_2_hand_schedule = ttk.Button(frame, text="#手動排缸",command=lambda:  [exeute_manual_schedule_task(),Switch_buttons(btn4_2_hand_schedule,btn5_output_csv)])
   ...
   #start_to_schedule_clear_cylinder_jobtask(_pro_joblist,_cy_ptr_list, _cy_group_list)
    ...
    btn4_3_bad_schedule = ttk.Button(frame, text="#排入清缸",command=lambda:  groupList_to_clear_cylinder_jobtask(processing_cylinder_list,cy_ptr_list, cy_group_list))
